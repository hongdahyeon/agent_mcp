...
export function OpenApiManager() {
    ...
    // 파라미터(JSON 스키마) 편집 방식: 'key-value' | 'raw'
    const [paramMode, setParamMode] = useState<'key-value' | 'raw'>('key-value');
    const [paramItems, setParamItems] = useState<{ key: string; value: string }[]>([{ key: '', value: '' }]);
    ...
    const fetchApis = useCallback(async (
        ...
        q: string = filterQ,
        tagObj: MetaItem | null = selectedFilterTag
    ) => {
        try {
            setLoading(true);
    ...
    const handleSave = async () => {
        ...
        try {
            const res = await fetch('/api/openapi', {
                ...
                body: JSON.stringify({
                    ...currentApi,
                    params_schema: paramMode === 'key-value' ? syncParamItemsToJson() : currentApi.params_schema,
    ...
    const openEditModal = (api: OpenApiConfig) => {
        ...
        // 파라미터 스키마 파싱하여 Key-Value 아이템으로 변환
        if (api.params_schema) {
            try {
                const parsed = JSON.parse(api.params_schema);
                const items = Object.entries(parsed).map(([k, v]) => ({ key: k, value: String(v) }));
                setParamItems(items.length > 0 ? items : [{ key: '', value: '' }]);
                setParamMode('key-value');
            } catch (e) {
                console.error("JSON Parse Error when opening modal:", e);
                setParamItems([{ key: '', value: '' }]);
                setParamMode('raw'); // 파싱 실패 시 원본 유지를 위해 raw 모드로 전환
            }
        } else {
            setParamItems([{ key: '', value: '' }]);
            setParamMode('key-value');
    ...
    // 파라미터 관련 유틸리티
    const addParamRow = () => setParamItems([...paramItems, { key: '', value: '' }]);
    const removeParamRow = (index: number) => {
        const next = paramItems.filter((_, i) => i !== index);
        setParamItems(next.length > 0 ? next : [{ key: '', value: '' }]);
    };
    const updateParamRow = (index: number, field: 'key' | 'value', value: string) => {
        const next = [...paramItems];
        next[index][field] = value;
        setParamItems(next);
    };

    // Key-Value 아이템을 JSON 문자열로 변환
    const syncParamItemsToJson = useCallback(() => {
                                ...
                                <tr key={api.id} className="hover:bg-gray-50 dark:hover:bg-slate-800/50 transition-colors">
                                    ...
                                    <td className="px-6 py-4">
                                        <div className="flex flex-wrap gap-1">
                                            {api.tags && Array.isArray(api.tags) ? api.tags.map((tag: string, idx: number) => (
                                    ...
                                    <div className="grid grid-cols-2 gap-4">
                                        <div className="space-y-1">
                                            ...
                                            <Autocomplete
                                                ...
                                                onSearch={async (q) => {
                                                    const res = await fetch(`/api/openapi/categories`, { headers: getAuthHeaders() });
                                                    const data: MetaItem[] = await res.json();
                                                    return data.filter((item: MetaItem) => item.name.includes(q));
                                                }}
                                                onCreate={async (name) => {
                                        ...
                                        <div className="space-y-1">
                                            <div className="flex justify-between items-center mb-2">
                                                ...
                                                <div className="flex bg-gray-100 dark:bg-slate-800 p-0.5 rounded-lg text-[10px]">
                                                    <button
                                                        type="button"
                                                        onClick={() => setParamMode('key-value')}
                                                        className={`px-2 py-1 rounded-md transition-all ${paramMode === 'key-value' ? 'bg-white dark:bg-slate-700 text-indigo-600 dark:text-indigo-400 shadow-sm font-bold' : 'text-gray-500 dark:text-slate-500'}`}
                                                    >
                                                        Key-Value
                                                    </button>
                                                    <button
                                                        type="button"
                                                        onClick={() => setParamMode('raw')}
                                                        className={`px-2 py-1 rounded-md transition-all ${paramMode === 'raw' ? 'bg-white dark:bg-slate-700 text-indigo-600 dark:text-indigo-400 shadow-sm font-bold' : 'text-gray-500 dark:text-slate-500'}`}
                                                    >
                                                        Raw JSON
                                                    </button>
                                                </div>
                                            ...
                                            {paramMode === 'key-value' ? (
                                                <div className="space-y-2 border border-gray-100 dark:border-slate-800 rounded-xl p-3 bg-gray-50/30 dark:bg-slate-900/10">
                                                    {paramItems.map((item, idx) => (
                                                        <div key={idx} className="flex gap-2 items-center">
                                                            <input
                                                                type="text"
                                                                placeholder="Key (ex: year)"
                                                                value={item.key}
                                                                onChange={(e) => updateParamRow(idx, 'key', e.target.value)}
                                                                className="flex-1 min-w-0 px-3 py-1.5 border border-gray-200 dark:border-slate-700 rounded-lg focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 outline-none transition-all text-xs font-mono bg-white dark:bg-slate-800 text-gray-900 dark:text-slate-100"
                                                            />
                                                            <input
                                                                type="text"
                                                                placeholder="Value (ex: 2024)"
                                                                value={item.value}
