...
- **Route Implementation (`src/routers/mcp_execution.py`)**:
  ...
  - **Authentication**: `Authorization: Bearer <token>` 헤더를 통한 통합 인증 지원 (JWT 및 외부 토큰).
...
## Phase 48: 외부 액세스 토큰별 도구 권한 관리 [Completed]

### Goal

관리자가 외부 액세스 토큰(`sk_...`)에 대해 특정 `Custom Tool` 및 `OpenAPI` 도구만 사용할 수 있도록 제어하는 세밀한 권한 시스템을 구축합니다.

### Implemented Changes

#### 1. Database Implementation

- **Tables**: `h_access_token_tool_map`, `h_access_token_openapi_map` 생성.
- **Migration**: 기존 토큰들에 대해 전체 권한을 부여하는 마이그레이션 (`src/db/migration_token_tools.py`) 수행.

#### 2. Backend Implementation

- **Automatic Mapping**: 신규 토큰 생성 및 신규 도구 등록 시 모든 토큰에 자동 권한 매핑 로직 추가.
- **Security Process**: 도구 실행 시 **[권한 확인 -> 횟수 체크 -> 실행]** 3단계 로직 적용 (`call_tool`, `api_execute_openapi`).
- **Management API**: 토큰별 권한 조회/수정 엔드포인트(`GET/POST /api/tokens/{token_id}/permissions`) 구현.

#### 3. Frontend Implementation

- **UI**: `AccessTokenManager.tsx` 내 '권한 설정' 버튼 및 상세 제어 모달(Custom/OpenAPI 분리) 구현.
- **Validation**: 잘못된 API 경로 수정 및 런타임 오류 방지 로직 적용.

### Verification Results

1. **Permission Test**: 관리자 화면에서 특정 도구 권한 해제 시 외부 REST API 접근 차단 및 `403 Forbidden` 반환 확인.
2. **Full-Allow Test**: 신규 토큰 생성 시 기본적으로 모든 권한이 부여되어 즉시 사용 가능한지 확인.

---

## Phase 49: 감사 로그 내보내기 (Export Audit Logs) [Completed]

### Goal

도구 사용 이력 및 OpenAPI 사용 이력을 CSV 및 Excel 파일로 내보낼 수 있는 기능을 구현하여 관리자의 데이터 분석 및 보관 편의성을 높입니다.

### Proposed Changes

#### 1. Backend Implementation [Completed]

- **Dependencies**: `requirements.txt`에 `pandas` 및 `openpyxl` 추가.
- **New Router (`src/routers/export.py`)**:
  - `POST /api/export/mcp/usage`: MCP 도구 사용 이력 내보내기 (Excel전용, CSV주석처리).
  - `POST /api/export/openapi/usage`: OpenAPI 사용 이력 내보내기 (Excel전용, CSV주석처리).
- **Server Integration (`src/server.py`)**: 신규 `export_router` 등록.

#### 2. Database Enhancement [Completed]

- **`src/db/mcp_tool_usage.py`**: 페이징 없는 이력 조회 함수 `get_all_tool_usage_logs` 추가.
- **`src/db/openapi_usage.py`**: 페이징 없는 이력 조회 함수 `get_all_openapi_usage_logs` 추가.

#### 3. Frontend Implementation [Completed]

- **`UsageHistory.tsx`**: "Excel 내보내기" 버튼 추가 (CSV 주석).
- **`OpenApiStats.tsx`**: "Excel 내보내기" 버튼 추가 (CSV 주석).

### Verification Plan

1. **Format Test**: 내보낸 CSV/Excel 파일이 엑셀 등에서 정상적으로 열리는지 확인.
2. **Data Integrity Test**: 화면에 표시되는 데이터와 내보낸 파일의 데이터가 일치하는지 확인.
3. **Filter Test**: (선택 사항) 현재 필터링된 결과만 내보낼 수 있도록 구현 시 필터 적용 여부 확인.

---

## Phase 50: Log Archiving & Unarchiving Policy [In-Progress]

### Goal

관리자가 과거 로그 파일을 선택하여 압축(Zip) 파일로 저장하고, 압축된 후에는 원본 파일을 목록에서 제거(삭제)하여 관리 편의성을 높입니다. 또한 압축 파일 내부를 확인하고 필요 시 다시 압축을 해제(Unzip)하여 복구하는 기능을 구현합니다.
...
- **`src/routers/system.py`**:
  - **`get_logs_list`**: `.txt` 파일뿐만 아니라 `.zip` 파일도 목록에 포함하여 반환. 파일 객체에 `type` ('text' | 'zip') 필드 추가.
  - **`api_archive_logs`**: 압축 성공 후 선택된 원본 `.txt` 파일들을 삭제하는 로직 추가.
  - **`api_get_zip_content` (`GET /api/system/logs/zip-content/{filename}`)**: Zip 파일 내의 파일 목록을 반환하는 신규 엔드포인트.
  - **`api_unzip_logs` (`POST /api/system/logs/unzip`)**: 지정된 Zip 파일을 해제하고 원본 로그 파일들을 복구한 뒤, Zip 파일은 삭제(또는 유지 선택)하는 엔드포인트.

#### 2. Frontend Implementation

- **`LogViewer.tsx`**:
  - **UI 개선**: 목록에서 텍스트 로그와 압축 로그를 아이콘으로 구분.
  - **Zip 클릭 시**: 파일 내용 대신 Zip 내부 파일 목록을 표시.
  - **압축 해제 버튼**: Zip 파일 선택 시 '압축 해제' 버튼 노출 및 기능 연동.
  - **자동 갱신**: 압축/해제 작업 후 목록을 자동으로 새로고침.
...
FastAPI의 내장 기능을 활용하여 MCP REST API Proxy 엔드포인트에 대한 Swagger(OpenAPI) 문서를 자동 생성하고 개선합니다. 외부 클라이언트가 사용 가능한 도구를 쉽게 파악하고 테스트할 수 있도록 메타데이터와 인터페이스를 강화합니다.
...
## Phase 52: Jenkins Telegram Integration [Completed]

### Goal

Jenkins CI/CD 파이프라인의 빌드 및 배포 결과를 텔레그램으로 실시간 통보하여 관리자가 어디서든 시스템 상태를 파악할 수 있도록 합니다.

### Implemented Changes

- **Jenkinsfile**: `post` 섹션에 `sendTelegramNotification` 함수를 추가하였습니다.
  - `withCredentials`를 사용하여 Jenkins에 저장된 `telegram-token`과 `telegram-chat-id`를 안전하게 로드합니다.
  - `try-catch` 블록을 적용하여 텔레그램 전송 실패가 전체 빌드 실패로 이어지지 않도록 방어적으로 설계했습니다.
  - 인코딩 문제를 방지하기 위해 알림 메시지는 영문으로 구성하였습니다.

---

## Phase 53: 백엔드 시스템 Telegram 연동 [Completed]

### Goal

백엔드 서버에서 발생하는 중요한 시스템 알림(사용량 임계치, 계정 잠금 등)을 기존 SSE 방식과 더불어 텔레그램으로도 동시에 발송합니다.

### Implemented Changes

- **Telegram Bot Module (`src/utils/telegram_bot.py`)**: `httpx` 라이브러리를 사용하여 텔레그램 API와 비동기적으로 통신하는 헬퍼 모듈을 구현했습니다.
- **Notification Helper (`src/utils/notification_helper.py`)**: `send_system_notification` 함수를 확장하여 SSE 브로드캐스트 후 텔레그램 전송 로직이 비동기로 실행되도록 통합했습니다.
- **Environment Config**: `.env` 파일을 통해 `TELEGRAM_BOT_TOKEN` 및 기본 `TELEGRAM_CHAT_ID`를 관리하도록 설정했습니다.

---

## Phase 54: 사용자별 Chat ID 지원 및 DB 확장 [Completed]

### Goal

개별 사용자마다 본인의 텔레그램으로 알림을 받을 수 있도록 DB 스키마를 확장하고, 기존 데이터와의 호환성을 위해 자동 마이그레이션 도구를 제공합니다.
...
## Phase 61: UI 레이아웃 하단 여백 및 스크롤 개선 [Completed]

### Goal

여러 관리 화면에서 마지막 카드가 바닥에 붙어 있어 답답해 보이는 현상을 해결하고, 고정 높이 제한으로 인해 하단 패딩이 소실되는 UI 버그를 수정하여 시각적 직관성과 사용성을 높입니다.
...
- **`LoginHistViewer.tsx`**: 고정 높이 해제 및 하단 여백 확보.
...
## Phase 64: 서버 로그 보관소 복사 기능 추가

### 목표

서버 로그 보관소에서 선택한 로그 파일의 내용을 한 번에 클립보드에 복사할 수 있는 기능을 추가하여 사용자 편의성을 높입니다.
...
- **[MODIFY] `src/frontend/src/components/functions/LogViewer.tsx`**:
  ...
  - 복사 성공 시 사용자에게 알림(alert 또는 UI 피드백) 제공.
