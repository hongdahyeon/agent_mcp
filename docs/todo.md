...
## [Pending] 향후 고도화 추진 과제 (Priority)

### 1. 보안 및 접근 제어 (Security)
...
### 3. 기능 확장 (Features)
- **cron 스케줄러 기능 구체화**: 테이블명, 대상 도구, 주기 설정 및 실행 결과의 정형화된 저장 로직 구현
- **부서/그룹 단위 관리**: 사용자를 그룹으로 묶고 그룹별로 도구 사용 권한 및 제한 설정
- **지식 베이스(RAG) 연동**: 프로젝트 내의 문서(`docs/`)를 학습하여 Agent가 답변에 참고할 수 있도록 하는 검색 도구 추가

### 4. 사용자 경험 및 분석 (UX & Analytics)
- **고도화된 분석 대시보드**: 도구별 평균 실행 시간, 성공률 추이, 피크 타임 분석 등 심층 통계 제공
- **다국어 지원 (i18n)**: 한국어 외에 영어 등 다양한 언어 선택 기능 추가
- **실시간 도구 실행 모니터링**: 실행 시간이 긴 도구의 경우, 진행 상태를 프로그레스 바 등으로 실시간 표시

### 5. 시스템 유지보수 (DevOps)
- **자동 DB 최적화**: 주기적인 SQLite `VACUUM` 수행 및 오래된 로그 아카이빙 기능

---

## [Recommendation] 신규 기능 제안 (Antigravity's Picks)

### 1. 지능형 서비스 운영 (AI Ops)
- **API 사용 비용 모니터링 (Usage Cost Tracking)**: LLM 호출에 따른 예상 비용 산출 및 대기 시간 통계 시각화
- **지능형 에러 진단 (Intelligent Error Diagnosis)**: 도구 호출 실패 시 LLM을 활용한 에러 원인 분석 및 가이드 자동 생성
- **사용자 피드백 루프 (User Feedback Loop)**: 도구 답변 품질에 대한 '좋아요/싫어요' 수집 및 성능 개선 데이터로 활용

### 2. 개발 및 보안 강화 (Dev & Security)
- **프롬프트 관리 및 플레이그라운드 (Prompt Management)**: 시스템 프롬프트 버전 관리 및 UI 기반 테스트 환경 제공
- **보안 감사 리포트 (Security Audit)**: 비정상적인 접근 패턴이나 반복적인 인증 실패에 대한 자동 브리핑 생성

---

## [History] 고도화 완료 과제 (Implemented)

### 1. 시스템 관리 및 안정성
- **DB 백업 및 복구**: 관리자 화면에서 SQLite DB 파일을 백업(다운로드)하고 복원하는 기능 (v)
- **헬스 체크(Health Check) 페이지**: DB 연결, SMTP, 스케줄러 상태를 한눈에 확인하는 진단 페이지 (v)
- **로그 정기 삭제 정책**: 설정된 기간이 지난 로그 파일을 자동으로 삭제하거나 압축 관리하는 기능 (v)

### 2. 인터페이스 및 데이터 시각화
- **다크 모드(Dark Mode) 지원**: 시스템 설정 또는 테마 스위처를 통한 다크 테마 전면 적용 (v)
- **알림 센터(Notification Center)**: 사용량 임계치 도달, 스케줄러 결과, 계정 잠금 등을 실시간 알림으로 제공 (v)
- **API 카테고리/태그 관리**: 도구가 많아질 경우를 대비한 그룹화 기능 (v)
- **사용량 히트맵(Heatmap)**: 시간대별/요일별 사용 패턴 시각화 (v)
- **사용자별 상세 분석**: 특정 사용자의主 사용 도구 TOP 5 분석 차트 추가 (v)

### 3. 권한 및 감사
- **도구별 접근 제어**: ROLE_ADMIN/ROLE_USER 수준을 넘어 특정 도구별 권한 부여 (v)
- **감사 로그 내보내기**: 도구 사용 및 접속 이력을 CSV/Excel 파일로 다운로드 (v)
- **API 문서화 자동화**: MCP REST API Proxy 엔드포인트에 대한 Swagger/OpenAPI 문서 자동 생성 (v)

### 4. 기타 완료 사항
- **h_openapi 문서 미리보기 및 PDF Export** (v)
