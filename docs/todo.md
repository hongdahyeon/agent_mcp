...
- 테이블명, 어떤 openapi tool, 주기 설정
- -> 해당 시기에 cron으로 실행
- -> 테이블에 데이터 저장 이때, 각 요소값을 묶어서 string 형태로 저장

### 7. 향후 추가 고도화 아이디어 (Proposed)

#### 1. 세밀한 권한 관리 (Granular RBAC)

- **도구별 접근 제어**: 현재 `ROLE_ADMIN`/`ROLE_USER` 수준을 넘어, 특정 도구별로 실행 권한을 부여하는 기능 (v)
- **부서/그룹 단위 관리**: 사용자를 그룹으로 묶고 그룹별로 도구 사용 권한 및 제한 설정

#### 2. 데이터 분석 및 감사 고도화

- **감사 로그 내보내기**: 도구 사용 이력 및 접속 이력을 CSV/Excel 파일로 다운로드하는 기능 (v)
