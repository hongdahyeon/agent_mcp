...
`http://{서버_IP}:8000/api/execute/{tool_id}?token={유효한_토큰}`

### 인증 (Authentication)
프록시 API 호출 시에는 반드시 이 시스템에서 발급한 유효한 토큰(`sk_...` 형식 또는 JWT)이 필요합니다.
- **방식**: Query Parameter
- **Key**: `token`
- **Example**: `.../api/execute/holiday_info?token=sk_abc123...`
