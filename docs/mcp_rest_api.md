...
- **Authentication**: JWT 또는 외부 보안 토큰 (`token` 쿼리 파라미터 또는 `Authorization: Bearer` 헤더)
...
**Request:**
- **URL**: `http://localhost:8000/api/mcp/proxy/add?token=발급받은_액세스_토큰`
- **Method**: `POST`
- **Body (JSON)**:
  ```json
  {
...
**Request:**
- **URL**: `http://localhost:8000/api/mcp/proxy/{커스텀_도구_이름}?token=...`
- **Method**: `POST`
- **Body (JSON)**:
  ```json
  {
